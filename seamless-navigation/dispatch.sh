...
tmux_prefix=${HERDR_TMUX_PREFIX:-}
if [ -z "$tmux_prefix" ] && command -v tmux >/dev/null 2>&1; then
  configured_prefix=$(tmux show-options -gv prefix 2>/dev/null || true)
  case "$configured_prefix" in
    C-*) tmux_prefix="ctrl+${configured_prefix#C-}" ;;
    M-*) tmux_prefix="alt+${configured_prefix#M-}" ;;
    ?*) tmux_prefix=$configured_prefix ;;
  esac
fi
tmux_prefix=${tmux_prefix:-ctrl+b}
...
case "$process_name" in
  ...
  tmux|tmate)
    case "$mode" in
      navigate) exec "$herdr_bin" pane send-keys "$pane_id" "$tmux_prefix" "$direction" ;;
      resize) exec "$herdr_bin" pane send-keys "$pane_id" "$tmux_prefix" "alt+$direction" ;;
    esac
    ;;
esac
