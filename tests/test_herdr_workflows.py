...
WORKSPACE_SCRIPT = BASE_ROOT / "last-workspace/workspace.py"
NAVIGATION_SCRIPT = BASE_ROOT / "seamless-navigation/dispatch.sh"
WORKSPACE_PICKER_SCRIPT = BASE_ROOT / "command-palette/workspace-picker.py"
PANE_TITLE_SCRIPT = BASE_ROOT / "pane-title/sync.py"
PANE_TITLE_WATCHER = BASE_ROOT / "pane-title/watch.py"
PREVIOUS_PANE_TRACKER = BASE_ROOT / "previous-pane-focus/track-focus.sh"
PREVIOUS_PANE_RESTORER = BASE_ROOT / "previous-pane-focus/restore-previous.sh"
...
class PaneTitleTest(unittest.TestCase):
    def test_combines_agent_and_terminal_title(self):
        ...
        request.assert_called_once_with(
            ...
            {
                "pane_id": "w1:p1",
                "source": "brettinternet.pane-title",
                "title": "omp · Fix pane headers",
                "tokens": {"brettinternet_pane_title": "1"},
            },
        )
    ...
    def test_custom_pane_label_overrides_automatic_title(self):
        ...
        request.assert_called_once_with(
            "pane.report_metadata",
            {
                ...
                "source": "brettinternet.pane-title",
                "clear_title": True,
                "tokens": {"brettinternet_pane_title": None},
            },
        )
...
class SeamlessNavigationTest(unittest.TestCase):
    ...
    def test_sends_standard_navigation_into_tmux(self):
        call = self.run_dispatch("tmux", "navigate", "right", "ctrl+l")
        self.assertEqual(call, "pane send-keys w1:p2 ctrl+b right")

    def test_sends_standard_resize_into_tmux(self):
        call = self.run_dispatch("tmux", "resize", "left", "alt+shift+h")
        self.assertEqual(call, "pane send-keys w1:p2 ctrl+b alt+left")
    ...
    def test_invokes_selected_action_with_cli_argument_order(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            herdr.write_text(
                ...
                "  printf '%s\\n' '{\"result\":{\"actions\":[{\"title\":\"Equalize\",\"plugin_id\":\"brettinternet.pane-equalize\",\"action_id\":\"equalize\"}]}}'\n"
                "  exit 0\n"
                "fi\n"
                f"printf '%s\\n' \"$*\" > '{log}'\n"
            )
            ...
            subprocess.run(
                ["sh", str(BASE_ROOT / "command-palette/palette.sh")],
                check=True,
                env=environment,
            )
            ...
            self.assertEqual(
                log.read_text().strip(),
                "plugin action invoke equalize --plugin brettinternet.pane-equalize",
            )
